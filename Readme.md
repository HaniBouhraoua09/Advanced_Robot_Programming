...
The simulation relies on a physics engine (`dynamics.c`) that models the drone as a point mass subject to various forces. The system uses **Euler Integration** to update the state at each time step $T$.

### A. Dynamics (Motion Equations)
The drone's motion is determined by Newton's Second Law ($F = ma$). The state update follows these discrete steps:

1.  **Acceleration:**
    $$a(t) = \frac{F_{total}(t)}{M}$$
2.  **Velocity Update:**
    $$v(t+T) = v(t) + a(t) \cdot T$$
3.  **Position Update:**
    $$p(t+T) = p(t) + v(t+T) \cdot T + \frac{1}{2} a(t) \cdot T^2$$

*(Where $M$ is mass, $T$ is the time step, $v$ is velocity, and $p$ is position).*

### B. Forces Calculation
The total force acting on the drone is the sum of three components:

$$F_{total} = F_{input} + F_{friction} + F_{repulsion}$$

#### 1. Input Force ($F_{input}$)
Generated by the user via the keyboard (Control Window). The keys apply a constant force vector in the desired direction.

#### 2. Viscous Friction ($F_{friction}$)
To simulate air resistance and prevent infinite acceleration, a damping force opposes the velocity:
$$F_{friction} = -K \cdot v(t)$$
*(Where $K$ is the friction coefficient defined in `params.txt`).*

#### 3. Repulsive Force ($F_{repulsion}$)
Based on **Artificial Potential Fields** (Latombe/Khatib model), obstacles and walls exert a repulsive force when the drone enters their influence radius ($\rho$).

$$F_{rep} = \begin{cases} \eta \left( \frac{1}{d} - \frac{1}{\rho} \right) \frac{1}{d^2} \nabla d & \text{if } d \le \rho \\ 0 & \text{if } d > \rho \end{cases}$$

This ensures the drone is "pushed away" from walls and obstacles dynamically, creating a collision-avoidance behavior.
